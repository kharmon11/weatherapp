...
- **`server/app/main.py`** — FastAPI app entrypoint. CORS origins come from `ALLOWED_ORIGINS` env var in production, hardcoded localhost otherwise; in production, `CANDIDATE_HOSTNAME_PATTERN` additionally allows the ephemeral non-promoted-deploy hostname shape (`sha-<hash>-dot-weatherapp-149500...appspot.com`) via CORS's `allow_origin_regex`, since that per-deploy hostname can never be added to the static `ALLOWED_ORIGINS` list (see `client/scripts/smoke-test.mjs` below for the analogous, unresolved gap on the Maps JS key side). When `ENV=production`, it additionally serves the built frontend as static files (mounted at `/`) plus an explicit `no-cache` route for `index.html` (for cache-busting on deploys). In dev, only the API runs — the frontend is served separately by Vite.
